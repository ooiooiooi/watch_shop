...
SLEEP = float(os.getenv("AVI_AUDIT_SLEEP", "0.25"))
...
def fetch_detail(url: str) -> Dict[str, object]:
    ...
    proc = subprocess.run([NODE_BIN, DETAIL, url], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, env=env, timeout=180)
...
def main() -> int:
    ...
    source_map = load_map()
    rows = mysql_rows(
        "SELECT id,name,brand,model,reference,price,image,images,description "
        "FROM product ORDER BY brand,id"
    ...
    for row in rows:
        ...
        local_missing = int(
            any(item.startswith("http://") or item.startswith("https://") for item in [image, *db_images])
        )
        summary["local_image_missing"] += local_missing
        source_url = source_map.get(product_id, "")
        status = "ok"
        src_name = ""
        src_price = ""
        src_ref = ""
        ...
        if not source_url:
            ...
            try:
                payload = fetch_detail(source_url)
        ...
        if len(report_rows) % 50 == 0:
            print(json.dumps({"processed": len(report_rows), **summary}, ensure_ascii=False), flush=True)
        time.sleep(SLEEP)

    csv_path = OUT_DIR / "source_audit.csv"
    json_path = OUT_DIR / "source_audit_summary.json"
